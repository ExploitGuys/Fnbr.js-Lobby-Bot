...
(async () => {
  ...
  //  if (currentLibVersion !== latestVersion) consoe.log('\x1b[31mWARNING: You\'re using an older version of the library. Please run installDependencies.bat\x1b[0m');
  let config;
  try {
  ...
  client.on('party:invite', (inv) => {
    ...
    console.log(`${config.inviteaccept ? 'accepted' : 'declined'} party invite from: ${inv.sender.diplayName}`);
  });
  ...
  const handleCommand = (message, sender) => {
    ...
    if (command === 'help') {
    ...
    } else if (command === 'backbling') {
      const skin = findCosmetic(content, 'outfit');
      if (skin) client.party.me.setBackpack(backpack.id);
      else message.reply(`Backbling ${content} wasn't found!`);
      ...
      client.party.me.setOutfit('CID_030_Athena_Commando_M_Halloween', [{ channel: 'ClothingColor', variant: 'Mat3' }]);
      ...
      client.party.me.setOutfit('CID_028_Athena_Commando_F', [{ channel: 'ClothingColor', variant: 'Mat2' }]);
      ...
      client.party.hideMembers(true)
    } else if (command === 'unhide') {
      client.party.hideMembers(false)
    } else if (command === 'copy') {
      const info = client.party.members.get(sender.id)
      client.party.me.setPickaxe(info.pickaxe);
      client.party.me.setOutfit(info.outfit);
      client.party.me.setBackpack(info.backpack);
      // client.party.me.setEmote(info.emote);
    }
  }
